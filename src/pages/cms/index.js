...
function Cms () {
 ...
 const columns = [
  ...
  },
  {
   ...
   render: data => {
    return (
     <Space size="middle">
      <Button type="primary" shape="circle" icon={<EditOutlined />} onClick={() => navigate(`/Admin/Artical?id=${data.id}`)} />
      <Popconfirm
       title="确认删除该条文章吗?"
 ...
 const delArticle = async (data) => {
  await httpHelper.delete(`/mp/articles/${data.id}`)
  // 更新列表
  setParams({
   page: 1,
   per_page: 5
 ...
 useEffect(() => {
  async function fetchArticleList () {
   const res = await httpHelper.get('/mp/articles', { params })
 ...
 return (
  <div>
   ...
   >
    <Form initialValues={{ status: -1, articleType: "" }} onFinish={onSearch}>
